...
\usepackage{amsmath, amsfonts, amssymb, array, graphicx, mathtools, tabularx}
...
\begin{itemize}
\item \textbf{Strategy}: \texttt{EngineInterface} è l'interfaccia a cui tutti i contenitori di algoritmi devono rifarsi per essere creati ed invocati da \texttt{Server}. \\
\item \textbf{Context}: \texttt{EnginePureFabricator} è il context - contiene un riferimento ad un Engine, \texttt{EngineInterface} è l'oggetto strategy, dichiara un’interfaccia comune agli algoritmi supportati mentre i vari *Engine sono i concrete strategy dato che implementano l’algoritmo usando strategy. \\
\item \textbf{ConcreteStrategy}: i vari engine contengono l'algoritmo per rispondere alle richieste. \\
\end{itemize}
\end{frame}

\begin{frame}{Strategy generico}
  \begin{columns}[c]
    \begin{column}{1\linewidth}
      \centering
      \includegraphics[height=0.80\textheight, keepaspectratio]{UMLGenerico.png}
    \end{column}
  \end{columns}
\end{frame}

\begin{frame}{Strategy specifico}
  \begin{columns}[c]
    \begin{column}{1\linewidth}
      \centering
      \includegraphics[width=\linewidth]{UMLSpecifico.png}
    \end{column}
  \end{columns}
\end{frame}
...
\subsection{Proxy}
\begin{frame}{Proxy}
\quad Il database SQL di java.sql agisce da proxy, assieme ad H2 \\
\begin{itemize}
...
\quad I test eseguiti su \texttt{GetPersonalDataEngine} sono tre per i due possibili metodi che potrebbero venir chiamati.
\begin{itemize}
...
\item \texttt{testPetitionerFailLogIn()} controlla solo che l'utente \texttt{Lancillotto.Benacense.99} non possa connettersi con le sue credenziali errate.
...
\begin{frame}{Replace Conditional with Polymorphism}
\quad Originalmente \texttt{Server} si occupava di scegliere quale oggetto \texttt{Engine} instanziare, tramite una lunga serie di condizioni.
Adesso questa responsabilità gli è stata rimossa ed attribuita ad \texttt{EnginePureFabricatorContext} che contiene un dizionario con alle chiavi i vari enum di \texttt{ComunicationType} e con un riferimento nei valori agli oggetti di tipo \texttt{EngineInterface} corrispondenti.  
\end{frame}
