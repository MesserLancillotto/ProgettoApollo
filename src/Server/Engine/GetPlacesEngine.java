...
public class GetPlacesEngine extends AuthenticatedEngine
{
    ...
    public GetPlacesEngine(String data) 
    {
        ...
        if (json.has("filters")) 
        {
            JSONObject filtersJSONObject = json.getJSONObject("filters");
            List<String> keys = new ArrayList<>(filtersJSONObject.keySet());
    ...
    {      
        if (!connectDB()) 
        {
            return new GetPlacesReply(false, null);
        } 
        try 
        {
        ...
        {
            e.printStackTrace();
            return new GetPlacesReply(true, null);
        }
    }
}